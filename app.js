/**
 * Stim Wallet by STM0x0, Main app.js
 * Module dependencies.
 */
...
const stim = require('stim-node-api');
...
recaptcha.init(process.env.CAPTCHA_PUBLIC, process.env.CAPTCHA_PRIVATE);
app.use(session({
  resave: true,
...
app.listen(app.get('port'), () => {
  console.log('%s Stim Wallet is running at http://localhost:%d in %s mode', chalk.green('✓'), app.get('port'), app.get('env')); 
